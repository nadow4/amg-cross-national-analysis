...
# This should match the dataset passed to data = ... in your global model
if (!exists("dat_model")) {
...
# 3) List variables used in your actual global / omnibus model
# These are the variables used in:
# AMG_Prev ~ GDP + Farm + DDD + Abx_Livestock + WWTP
...
# Optional: include additional outcomes only if they exist
if ("TetM" %in% names(dat_model)) {
  vars_used <- c(vars_used, "TetM")
}

if ("Richness" %in% names(dat_model)) {
  vars_used <- c(vars_used, "Richness")
}
...
if ("Shannon" %in% names(dat_model)) {
  vars_used <- c(vars_used, "Shannon")
}
...
vars_used <- unique(vars_used)

# 4) Confirm variables exist
missing_vars <- setdiff(vars_used, names(dat_model))

if (length(missing_vars) > 0) {
  cat("\nAvailable columns in dat_model are:\n")
  print(names(dat_model))
  
  stop("These vars are missing from dat_model: ", paste(missing_vars, collapse = ", "))
}

cat("\nAll expected model variables are present.\n")
...
# 6) Factor sanity checks
# Dredge/model fitting can break if a factor has only 1 level after filtering
...
# 7) How many complete cases exist for the core model?
# Use only one outcome at a time here to avoid dropping rows unnecessarily
...
cc_n_any <- sum(stats::complete.cases(dat_model[, core_vars_any, drop = FALSE]))

cat("\nComplete cases for Any ARG model vars:", cc_n_any, "of", nrow(dat_model), "\n")

# Optional: create a complete-case dataset for the Any ARG global model
dat_model_cc <- dat_model[stats::complete.cases(dat_model[, core_vars_any, drop = FALSE]), ]

cat("dat_model_cc dim:\n")
print(dim(dat_model_cc))

# Optional: make Farm a factor if it is being used as binary/categorical
if ("Farm" %in% names(dat_model_cc)) {
  dat_model_cc$Farm <- factor(dat_model_cc$Farm)
}
...
# ---- 1) Global models + dredge setup -----------------------------------------

# MuMIn::dredge requires na.action = na.fail.
# This must be set BEFORE the global models are fit.
old_na_action <- getOption("na.action")
options(na.action = "na.fail")
on.exit(options(na.action = old_na_action), add = TRUE)

predictors <- c("GDP", "Farm", "DDD", "Abx_Livestock", "WWTP")

make_model_data <- function(response) {
  vars_needed <- c(response, predictors)
  
  summary_df %>%
    dplyr::select(dplyr::all_of(vars_needed)) %>%
    dplyr::filter(stats::complete.cases(.)) %>%
    dplyr::mutate(Farm = factor(Farm))
}

dat_any  <- make_model_data("AMG_Prev")
dat_tetM <- make_model_data("TetM")
dat_rich <- make_model_data("Richness")
dat_shan <- make_model_data("Shannon")
...
cat("\nModel sample sizes:\n")
cat("AMG_Prev:", nrow(dat_any), "\n")
cat("TetM:", nrow(dat_tetM), "\n")
cat("Richness:", nrow(dat_rich), "\n")
cat("Shannon:", nrow(dat_shan), "\n")
...
################################################################################
## Manual AICc table for omnibus models using professor-requested K = 6
################################################################################
...
# Standardize Richness name if needed
if ("richness" %in% names(dat_model) && !"Richness" %in% names(dat_model)) {
  dat_model <- dat_model %>%
    mutate(Richness = richness)
}

# Make Farm categorical if appropriate
dat_model <- dat_model %>%
...
# Model list
omnibus_models <- list(
  "Any ARG"  = m_any_omnibus,
  "tet(M)"   = m_tetm_omnibus,
  "Richness" = m_richness_omnibus
)

# Manually use professor/table K = 6
K_manual <- rep(6, length(omnibus_models))

# Extract n and AIC
n_values <- sapply(omnibus_models, nobs)
AIC_values <- sapply(omnibus_models, AIC)

# Manual AICc using K = 6
AICc_values <- AIC_values + ((2 * K_manual * (K_manual + 1)) / (n_values - K_manual - 1))

# Delta AICc and Akaike weights
Delta_values <- AICc_values - min(AICc_values, na.rm = TRUE)

wi_values <- exp(-0.5 * Delta_values) / sum(exp(-0.5 * Delta_values), na.rm = TRUE)

# Final table
omnibus_aicc_table <- tibble(
  Variable = names(omnibus_models),
  Model = "Omnibus",
  Model_Composition = c(
    "ARG ~ GDP + Farm + DDD + Abx_Livestock + WWTP",
    "TetM ~ GDP + Farm + DDD + Abx_Livestock + WWTP",
    "Richness ~ GDP + Farm + DDD + Abx_Livestock + WWTP"
  ),
  n = n_values,
  ...
  Delta = round(Delta_values, 2),
  wi = round(wi_values, 3)
)
...
```{r}
################################################################################
## Figure 4: Top model plots for Any ARG and tet(M) only
## Side-by-side panels with country labels, 95% CI, R² and p-value subtitles
################################################################################
...
# ------------------------------------------------------------------
# Pull dredge-ranked top models
# ------------------------------------------------------------------
m_any <- MuMIn::get.models(dd_any,  subset = 1)[[1]]
m_tet <- MuMIn::get.models(dd_tetM, subset = 1)[[1]]
...
title_lab <- function(var) {
  dplyr::recode(var,
    "AMG_Prev" = "Any ARG prevalence",
    "TetM"     = "tet(M) prevalence",
    .default   = var
  )
}
...
# ------------------------------------------------------------------
# Helper: pick primary predictor from model
# ------------------------------------------------------------------
pick_x <- function(m, dat) {
  trms <- attr(terms(m), "term.labels")
  if (length(trms) == 0) return(NA_character_)
...
# ------------------------------------------------------------------
# Helper: compute R² and p-value
# ------------------------------------------------------------------
get_r2_p <- function(m, x_var) {
  sm <- summary(m)
  
  # p-value for predictor
  p <- if (!is.na(x_var) && x_var %in% rownames(sm$coefficients)) {
    sm$coefficients[x_var, 4]
  } else {
    NA_real_
  }
  
  # R² for lm models; deviance pseudo-R² for glm models
...
# ------------------------------------------------------------------
# Helper: build prediction grid + 95% CI
...
plot_model <- function(m, y_var, dat) {
  ...
  stats <- get_r2_p(m, x_var)
  
  r2_txt <- if (is.finite(stats$r2)) {
    sprintf("%.2f", stats$r2)
  } else {
    "NA"
  }
  
  p_txt <- if (is.finite(stats$p)) {
    if (stats$p < 0.001) {
      "< 0.001"
    } else {
      sprintf("%.3f", stats$p)
  ...
  subtitle_txt <- paste0("R² = ", r2_txt, ", p = ", p_txt)
  ...
  ggplot(d, aes(x = x, y = y)) +
    geom_point(size = 3.5, color = cols$point) +
    geom_ribbon(
      data = grid,
      aes(x = .data[[x_var]], ymin = lwr, ymax = upr),
      inherit.aes = FALSE,
...
# ------------------------------------------------------------------
# Combine side by side and label panels A and B
# ------------------------------------------------------------------
fig4 <- patchwork::wrap_plots(p_any, p_tet, ncol = 2) +
...
```{r}
################################################################################
## Stewardship stacked bars (0–5) + ARG overlays:
## 1) Any ARG (squares) + 2) TetM (triangles)
## Left axis  = stewardship score
## Right axis = ARG outcome proportion/prevalence
## Subtitle = R² and p for both outcomes ~ Weighted_0_25
################################################################################
...
if (length(miss_cols) > 0) {
  warning("No color defined for: ", paste(miss_cols, collapse = ", "))
}

# ---- Weighted score per country ----------------------------------------------
# Used for ordering countries and model subtitle only, not plotted directly
weighted_df <- stewardship_long %>%
  mutate(
    Weight  = unname(domain_weights[Domain]),
    contrib = Score * Weight
  ...
  summarise(
    ...
    Weighted_0_25 = 25 * sum(contrib, na.rm = TRUE),
    .groups = "drop"
  )

# ---- Join outcomes for overlays ----------------------------------------------
overlay_df <- summary_df %>%
  select(Country, AMG_Prev, TetM) %>%
  left_join(
    weighted_df %>% select(Country, Weighted_0_25),
    by = "Country"
  )
...
# ---- Scale outcomes to left axis height --------------------------------------
# Assumes AMG_Prev and TetM are proportions from 0 to 1.
# Multiplying by 5 maps them onto the stewardship axis, which ranges from 0 to 5.

overlay_df <- overlay_df %>%
  mutate(
    AMG_scaled  = AMG_Prev * 5,
    TetM_scaled = TetM * 5,
    AMG_label   = sprintf("%.2f", AMG_Prev),
    TetM_label  = sprintf("%.2f", TetM)
  )
...
subtitle_txt <- paste0(
  "AMG_Prev ~ Total: R² = ", sprintf("%.2f", r2_any),
  ", p = ", sprintf("%.2f", p_any),
  " | TetM ~ Total: R² = ", sprintf("%.2f", r2_tetm),
  ", p = ", sprintf("%.2f", p_tetm)
)
...
ggplot(stewardship_long, aes(x = Country, y = Score, fill = Domain_lab)) +
  ...
  # Left axis = stewardship, right axis = ARG/tet(M)
  scale_y_continuous(
    limits = c(0, 5.3),
    breaks = 0:5,
    name = "Stewardship score, unweighted total = 5",
  ...
  labs(
    title = "Stewardship domains with Any ARG and tet(M) overlays",
    subtitle = subtitle_txt,
    x = "Country, ordered by weighted stewardship score",
    fill = "Domain"
  ) +
...
make_stew_model_data <- function(response) {
  vars_needed <- c(response, stew_predictors)
...
dat_any_stew  <- make_stew_model_data("AMG_Prev")
dat_tetm_stew <- make_stew_model_data("TetM")
...
cat("\nStewardship model sample sizes:\n")
cat("AMG_Prev:", nrow(dat_any_stew), "\n")
cat("TetM:", nrow(dat_tetm_stew), "\n")
...
```{r}
################################################################################
## AICc, Delta, and Akaike weights for stewardship-domain omnibus models
## Uses stewardship domain columns already present in summary_df
################################################################################
...
dat_stew <- summary_df %>%
  mutate(
    VetRx = as.numeric(Veterinary_Prescription_Requirement),
    OTC   = as.numeric(OTC_Ban_Humans_Animals),
    HumRx = as.numeric(Human_Prescription_Requirement),
    Enf   = as.numeric(Enforcement_and_Monitoring),
    Adv   = as.numeric(Advanced_Stewardship)
  )
...
missing_vars <- setdiff(vars_needed, names(dat_stew))

if (length(missing_vars) > 0) {
  stop("These variables are missing from dat_stew: ", paste(missing_vars, collapse = ", "))
}

cat("\nNA counts:\n")
print(sapply(dat_stew[, vars_needed, drop = FALSE], function(x) sum(is.na(x))))

cat("\nModel data preview:\n")
print(dat_stew %>% select(all_of(vars_needed)))

# Complete-case dataset
dat_stew_cc <- dat_stew %>%
  filter(
    complete.cases(
      AMG_Prev,
      TetM,
...
cat("\nRows available for stewardship-domain omnibus models:", nrow(dat_stew_cc), "\n")

if (nrow(dat_stew_cc) == 0) {
  stop("No complete cases available. Check NA counts and stewardship column names.")
}

# ------------------------------------------------------------------------------
# 3. Fit stewardship-domain omnibus models
# ------------------------------------------------------------------------------

m_any_stew_omnibus <- lm(
  AMG_Prev ~ VetRx + OTC + HumRx + Enf + Adv,
  data = dat_stew_cc
)

m_tetm_stew_omnibus <- lm(
  TetM ~ VetRx + OTC + HumRx + Enf + Adv,
  data = dat_stew_cc
)

stew_omnibus_models <- list(
  "Any ARG" = m_any_stew_omnibus,
  "tet(M)"  = m_tetm_stew_omnibus
)

# ------------------------------------------------------------------------------
# 4. Manual AICc using table-defined K = 6
# ------------------------------------------------------------------------------

n_values <- sapply(stew_omnibus_models, nobs)

# Your table-defined K:
# 5 domain predictors + intercept = 6
K_values <- rep(6, length(stew_omnibus_models))

AIC_values <- sapply(stew_omnibus_models, AIC)

AICc_values <- AIC_values +
  ((2 * K_values * (K_values + 1)) / (n_values - K_values - 1))

Delta_values <- AICc_values - min(AICc_values, na.rm = TRUE)

wi_values <- exp(-0.5 * Delta_values) /
  sum(exp(-0.5 * Delta_values), na.rm = TRUE)

# ------------------------------------------------------------------------------
# 5. Final table
# ------------------------------------------------------------------------------

stew_omnibus_aicc_table <- tibble(
  Variable = names(stew_omnibus_models),
  Model = "Omnibus",
  Model_Composition = c(
    "ARG ~ VetRx + OTC + HumRx + Enf + Adv",
    "TetM ~ VetRx + OTC + HumRx + Enf + Adv"
  ),
  n = n_values,
  K = K_values,
  ...
  AICc = round(AICc_values, 2),
  Delta = round(Delta_values, 2),
  wi = round(wi_values, 2)
)

stew_omnibus_aicc_table
```
...
################################################################################
## LN model plots: Any ARG ~ Enforcement_and_Monitoring; TetM ~ OTC Ban
## Points + fitted line + 95% CI + larger country labels + R² + p
...
get_r2_p_single <- function(m, x_var) {
  sm <- summary(m)
  p  <- if (x_var %in% rownames(sm$coefficients)) {
    sm$coefficients[x_var, 4]
  } else {
    NA_real_
  }
...
plot_ln_numeric <- function(dat, y_var, x_var, model, title, xlab, ylab,
  ...
  d <- dat %>%
    select(Country, all_of(y_var), all_of(x_var)) %>%
    rename(
      y = all_of(y_var),
      x = all_of(x_var)
    )
  ...
  ggplot(d, aes(x = x, y = y)) +
    ...
    geom_point(size = 3.5, color = point_col) +
    ggrepel::geom_text_repel(
      aes(label = Country),
      size = label_size,
      max.overlaps = Inf,
...
p_any_enforce <- plot_ln_numeric(
  ...
  x_var = "Enforcement_and_Monitoring",
  model = m_any_enforce,
  title = "Any ARG ~ Enforcement & Monitoring",
  xlab  = "Enforcement & Monitoring score",
  ylab  = "Any ARG prevalence",
  point_col = "#1f77b4",
  line_col  = "#d62728",
  fill_col  = "#1f77b4",
  label_size = 5
...
# ---- Side-by-side figure ------------------------------------------------------
p_ln_side_by_side <- p_any_enforce + p_tetm_otc +
  plot_layout(ncol = 2) +
  plot_annotation(
    title = "Linear model relationships between stewardship predictors and ARG outcomes",
    theme = theme(
      plot.title = element_text(face = "bold", size = 18, hjust = 0.5)
    )
  )

print(p_ln_side_by_side)

# Optional: save figure
ggsave(
  filename = "LN_model_ARG_stewardship_side_by_side.png",
  plot = p_ln_side_by_side,
  width = 14,
  height = 6.5,
  dpi = 300
)
...
```{r}
################################################################################
## GDP as predictor of stewardship scores
################################################################################

library(dplyr)
library(broom)

# GDP predicting stewardship domains
m_gdp_enforce <- lm(Enforcement_and_Monitoring ~ GDP, data = summary_df)
m_gdp_otc     <- lm(OTC_Ban_Humans_Animals ~ GDP, data = summary_df)

# Summaries
summary(m_gdp_enforce)
summary(m_gdp_otc)

# Clean model output
gdp_stewardship_results <- bind_rows(
  tidy(m_gdp_enforce) %>%
    mutate(
      Outcome = "Enforcement and Monitoring",
      Model = "Stewardship ~ GDP"
    ),
  tidy(m_gdp_otc) %>%
    mutate(
      Outcome = "OTC Ban Humans/Animals",
      Model = "Stewardship ~ GDP"
    )
) %>%
  select(Outcome, Model, term, estimate, std.error, statistic, p.value)

gdp_stewardship_results
```


GDP was significantly and positively associated with both stewardship indicators. This suggests that countries with greater economic capacity tended to have stronger antimicrobial stewardship infrastructure and policy implementation. However, because these models are cross-sectional, these findings should be interpreted as associations rather than evidence that GDP directly causes stronger stewardship.

```{r}
################################################################################
## GDP predicting stewardship indicators
## Enforcement & Monitoring ~ GDP
## OTC Ban Humans/Animals ~ GDP
## Points + fitted line + 95% CI + country labels + R² + p
## Side-by-side plot
################################################################################
...
plot_gdp_stewardship <- function(dat, y_var, x_var, model, title, xlab, ylab,
                                 point_col = "#1f77b4",
                                 line_col  = "#d62728",
                                 fill_col  = "#1f77b4",
                                 label_size = 5) {
...
p_gdp_enforce <- plot_gdp_stewardship(
  dat   = summary_df,
  y_var = "Enforcement_and_Monitoring",
  x_var = "GDP",
  model = m_gdp_enforce,
  title = "Enforcement & Monitoring ~ GDP",
  xlab  = "GDP",
  ylab  = "Enforcement & Monitoring score",
  point_col = "#1f77b4",
  line_col  = "#d62728",
  fill_col  = "#1f77b4",
  label_size = 5
...
p_gdp_otc <- plot_gdp_stewardship(
  dat   = summary_df,
  y_var = "OTC_Ban_Humans_Animals",
  x_var = "GDP",
  model = m_gdp_otc,
  title = "OTC Ban ~ GDP",
  xlab  = "GDP",
  ylab  = "OTC Ban Humans/Animals score",
  point_col = "#2ca02c",
  line_col  = "#9467bd",
  fill_col  = "#2ca02c",
  label_size = 5
...
p_gdp_stewardship_side_by_side <- p_gdp_enforce + p_gdp_otc +
  plot_layout(ncol = 2) +
  plot_annotation(
    title = "GDP as a predictor of antimicrobial stewardship indicators",
    theme = theme(
      plot.title = element_text(face = "bold", size = 18, hjust = 0.5)
    )
  )

print(p_gdp_stewardship_side_by_side)

# ---- Optional: save figure ---------------------------------------------------

ggsave(
  filename = "GDP_stewardship_side_by_side.png",
  plot = p_gdp_stewardship_side_by_side,
  width = 14,
  height = 6.5,
  dpi = 300
)
...
```{r}
################################################################################
## Transposed thesis table
## Countries as columns, variables as rows
################################################################################
...
# ---- Create country-site metadata table first --------------------------------

# Pull sampling-site longitude/latitude from coords_list.
# In coords_list, the first two values are the sampling site lon/lat.
site_coords <- tibble::tibble(
  Country = names(coords_list),
  Longitude = purrr::map_dbl(coords_list, ~ .x[1]),
  Latitude  = purrr::map_dbl(coords_list, ~ .x[2])
)

# If a site/city column exists in amg_data, use it.
# Otherwise, use Country as a safe placeholder.
possible_site_cols <- c("City/Site", "City_Site", "Site", "City", "Location")
site_col <- intersect(possible_site_cols, names(amg_data))[1]

if (!is.na(site_col)) {
  site_lookup <- amg_data %>%
    dplyr::distinct(Country, .keep_all = TRUE) %>%
    dplyr::transmute(
      Country = Country,
      `City/Site` = as.character(.data[[site_col]])
    )
} else {
  site_lookup <- summary_df %>%
    dplyr::transmute(
      Country = Country,
      `City/Site` = stringr::str_to_title(stringr::str_to_lower(Country))
    )
}

country_site_table <- summary_df %>%
  dplyr::select(
    Country,
    Sample_Size,
    Co_Pop,
  ...
  dplyr::left_join(site_lookup, by = "Country") %>%
  dplyr::left_join(site_coords, by = "Country") %>%
  dplyr::transmute(
    Country,
    `City/Site`,
    Longitude = round(Longitude, 4),
    Latitude = round(Latitude, 4),
    `Birds sampled (n)` = Sample_Size,
    `Country population` = round(Co_Pop, 0),
    `GDP per capita` = round(GDP_pc, 2),
    `DDD per capita` = round(DDD_pc, 6),
    `Abx livestock (mg/kg)` = round(Abx_mg_kg_raw, 2),
    `Meat consumption per capita` = round(Meat_pc, 6),
    `Farm site` = as.character(Farm),
    `Distance to WWTP (km)` = round(WWTP_Distance_km, 2),
    `Cumulative stewardship score` = round(Total_Score, 2)
  )

# ---- Create transposed table -------------------------------------------------

country_site_table_transposed <- country_site_table %>%
  mutate(
...
country_site_table_transposed


# ---- Word-safe thesis table --------------------------------------------------

country_site_table_transposed %>%
  knitr::kable(
    caption = "Sampling site characteristics, ecological covariates, antimicrobial use indicators, and cumulative stewardship scores by country.",
    align = "l"
  )
...
readr::write_csv(
  country_site_table_transposed,
  "country_site_metadata_table_transposed.csv"
)
```
